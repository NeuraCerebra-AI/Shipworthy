...
class GauntletFixtureTests(unittest.TestCase):
    ...
    def test_material_fixture_controls_are_oracled_and_files_stay_bounded(self) -> None:
        ...
        self.assertIn('function projectNameOrError()', script)
        self.assertIn('$("#toast").textContent = "Name is required"', script)
        self.assertIn('$("#project-name")?.addEventListener("input", () => { $("#toast").textContent = ""; })', script)
