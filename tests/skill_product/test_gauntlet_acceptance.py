...
class AcceptanceHarnessTests(unittest.TestCase):
    ...
    def test_briefs_name_exact_skills_and_do_not_expose_oracle(self) -> None:
        ...
        for name in ("runtime-only.md", "full-evidence.md"):
            ...
            self.assertIn("Pre-verifier closure check", text)
            self.assertIn("each observed input mechanism", text)
            self.assertIn("each reload/re-entry control and transition", text)
            self.assertIn("one finding and semantic effect code per defect class", text)
            self.assertIn("final readiness-ledger.json and report-input.json", text)
            self.assertIn("readiness-ledger.json contains the canonical top-level `path_frontier`", text)
            self.assertIn("report-input.json is exactly the `shipworthy/readiness-report-input` 1.0 wrapper", text)
            self.assertIn("`source_ledger` must be structurally identical", text)
            self.assertIn("safe relative paths to existing non-empty files under the evidence output", text)
            self.assertNotIn("must each contain the same top-level `path_frontier`", text)
