...
Before the verifier approves or HTML is rendered: Re-open both final JSON files from disk, readiness-ledger.json and report-input.json. readiness-ledger.json contains the canonical top-level `path_frontier`; report-input.json is exactly the `shipworthy/readiness-report-input` 1.0 wrapper, and its `source_ledger` must be structurally identical to readiness-ledger.json. Do not add a second top-level frontier or ad hoc checkpoint to the closed report wrapper. Every `Fix` finding needs non-empty `affected_semantic_keys`, a semantic `observed_effect_code`, and `evidence_refs`; evidence references are safe relative paths to existing non-empty files under the evidence output, optionally followed by a fragment. Every non-intent frontier row names its correct immediate parent and mechanically derives its semantic key from that parent. Validate both exact shapes against the bundled schemas, compare frontier identities/counts/closure and finding lineage, then render HTML from that exact report input. Missing or divergent canonical data fails completion.
