...
A full Shipworthy invocation is not a quick pass. Full blast means source-backed discovery of the bounded safe path universe, actual frontend execution, mandatory multi-wave evidence gates, adaptive continuation while safe work remains, persistent goal-mode execution when the platform and user authorization allow it, and a mandatory HTML report generated from the final ledger. If the candidate inventories, coverage manifest, path_frontier, coverage matrix, verifier certificates, evidence debt register, goal-mode status, and HTML report path are missing, the run is incomplete.
...
> Shipworthy full blast is a long-running audit. Recommended: reply yes to authorize persistent goal mode and parallel subagents for this Shipworthy run. If authorized, I will complete every required wave and verifier gate, use every applicable safe alternative when a capability fails, resume unfinished work from the checkpoint, and produce the final HTML report. The goal will not be marked complete while required work remains unfinished; any terminal limitation will identify what remains unfinished and why.

Use that complete block verbatim as one message. Do not shorten it, split it, paraphrase it, or substitute an older authorization sentence.
...
Keep operational lifecycle, coverage qualification, and release decision separate. In `orchestration-checkpoint.json`, record `audit_status` as `active`, `complete`, `blocked`, or `user_stopped`; keep `goal_mode_status` for goal availability/authorization; and record `goal_completion_status` as `active`, `complete`, `blocked`, `user_stopped`, or `not_applicable`. In `path_frontier`, record `finality: open|exhausted`, `remaining_safe_work`, and `resume_conditions`. In the ledger, use `readiness_disposition` only for `ready`, `conditionally_ready`, `not_ready`, or `cannot_determine`. An audit can complete with `not_ready`; a confirmed defect is not unfinished audit work.

Before claiming closure, reconcile candidate source extracts, `control_census_paths`, `zero_yield_pass_ids`, raw outputs, evidence-debt actions, recovery/failover state, and validation receipts through `scripts/render_report.py`. The orchestrator must not mark the persistent goal complete unless that final gate succeeds with `audit_status: complete`, a complete ledger, a closed multi-source frontier, full frontend path-walk, approved verifier, source-backed inventory/census reconciliation, qualifying zero-yield passes, a renderer-issued completion receipt, and no active recovery. A complete audit may still be `not_ready` and retain a terminal blocked recovery; a truthful `active`, `blocked`, or `user_stopped` audit still receives the mandatory HTML report but is not full closure.
...
This is an **inline-chat requirement**. Put the Remaining Work summary and the two questions directly in the assistant's user-visible `final` response. The HTML report, ledger, `remaining-work.json`, an attachment, or a link may provide the exhaustive detail, but none of them satisfies or replaces the inline chat handoff. The inline response must still name exact grouped counts, every unresolved category, the next frontier batch, the exhaustive artifact path when used, and the applicable two-question block.
...
5. Treat the run as read-only unless the user explicitly authorizes the exact action and a verified non-production reset/sandbox contract exists. A disposable fixture alone does not grant authority. High-risk actions require action-level approval plus the verified boundary.
...
Every controller or lane prompt that authorizes browser access must carry the Browser Failover Contract and Block-Recovery Ladder from `references/browser-evidence-routing.md`. A browser or agent failure does not end its wave. Preserve the unfinished frontier, apply the bounded recovery ladder, and resume the unfinished wave from the checkpoint when recovery succeeds. Supporting source, CLI, API, tests, and logs may explain the failure but never substitute for required frontend execution. Keep recovery `active` while an applicable safe authorized alternative remains; use `blocked` only after the final capability inventory proves exhaustion, and use `user_stopped` only when the user stops the run.

In particular, do not stop the lane before attempting already available independent Playwright; control through the same locked native-browser binding is not independent Playwright.
...
Use adaptive continuation after Wave 3 when coverage is not closed and safe authorized available work can still improve it. Continue with additional waves for unresolved major route families, user roles, state variants, runtime proof, contradictions, high-severity disconfirmation, safe path attempts, or evidence debt, or when the verifier objects that coverage is too thin. Do not end a full blast because three waves happened; exhaust it when no safe authorized available action can improve the evidence, then report either complete coverage or the exact constrained qualification and decision.

Path-universe accounting is mandatory. Read `references/coverage-manifest.md` and build source-backed candidate inventories before execution. Every material candidate must map to a canonical row labeled `covered`, `sampled_with_justification`, `blocked`, `avoided`, `missing`, `out_of_scope`, or `evidence_debt` before final readiness language is written. `unattempted`, `unknown`, and `maybe` remain nonterminal. Do not use `sampled` or `inferred` in a full frontier. Treat all safe discoverable paths inside the declared boundary as the coverage ambition; do not substitute a few happy paths, top tasks, or agent consensus.
...
Before collection, read `references/coverage-manifest.md`, `references/evidence-state.md`, `references/schemas/readiness-ledger.schema.json`, and `ship-product-workflows/references/path-discovery-and-coverage.md`. Maintain one canonical `path_frontier` using the readiness-ledger schema; do not accept an ad hoc list or a second frontier shape from a lane.
...
Each `path_frontier` row must use versioned semantic lineage from intent through feature, surface, control, and transition; record observations from canonical method families; carry attempt/evidence proof; and name an owner lane when material. Candidate inventories must retain raw locators and map one-to-one into the frontier; copying frontier keys into a census is circular and does not prove coverage. Valid terminal statuses are `covered`, `sampled_with_justification`, `blocked`, `avoided`, `missing`, `out_of_scope`, and `evidence_debt`.

Affirmative readiness is forbidden while any material path_frontier row remains `unattempted`, `unknown`, or `maybe`. Continue after the minimum three waves while `finality: open`. Exhaustion requires every material row to have a terminal status, runtime/static/declared candidate inventories to reconcile, and two qualifying zero-yield discovery passes from distinct canonical method families to find no new raw candidates. Before final synthesis, run a verifier pass asking "what plausible paths were missed?" If that pass names a plausible material path, add it to the candidate inventory and frontier, then continue or report a constrained decision. A proven missing path or tested failure produces `not_ready`; it does not keep the audit operationally active.
...
If a safe authorized available action remains, keep `finality: open`, set `exhaustion_status: incomplete`, list the next frontier batch, and continue or persist a resume checkpoint. When no safe authorized available action can improve coverage, use `finality: exhausted`, retain exact resume conditions and terminal dispositions, reconcile the evidence, and render the HTML report. The audit may finish `not_ready` or `cannot_determine`; only affirmative readiness requires fully proven coverage.
...
10. Execute the mandatory sequence in `references/coverage-manifest.md`: freeze declared/static/runtime candidate inventories with raw locators and digests; map each candidate exactly to one canonical frontier row or retained difference; walk safe spawned surfaces to a fixpoint; reconcile lifecycle, role, state, viewport, input-mode, recovery, and persistence variants; then run independent negative inventory passes. Build the material-state control census as supporting evidence, never by copying frontier keys and never as omission proof. Treat "try every single path" as every safe discoverable path in the bounded union of those inventories, with honest reasons and debt for anything unsafe or unavailable. Archetype overlays add provenance-tagged hypotheses to the declared inventory, never passes.
...
12. Run the Adaptive Exhaustion Gate after each wave: update candidate inventories, manifest, frontier burn-down, and new candidate IDs; retarget from unresolved rows and inventory differences; keep `finality: open` while safe authorized work remains. Use `exhausted` only when none remains, with exact resume conditions.
...
14. Maintain one shared evidence state: candidate inventories, canonical coverage manifest, claim ledger, coverage matrix, path_frontier, evidence debt register, path traces, evidence inventory, clarity lane packets, false positives, rejected claims, blocked checks, avoided checks, missing paths, and untested paths. Every full-frontier material row must use the canonical terminal or nonterminal taxonomy from `references/coverage-manifest.md`.
15. If the available evidence is static only, such as screenshots, README, docs, package scripts, or source snippets with no runnable runtime, route as a static constrained pass. Keep runtime behavior, persistence, accessibility, deployability, and path completion claims provisional, blocked, missing, or evidence debt unless directly proven.
...
20. Run the Raw-Evidence-to-Ledger Reconciliation Gate, then have the final no-overclaim verifier independently regenerate candidate inventories before comparing raw locators/mappings with the manifest. Fail a full-run claim for any unmapped candidate/observation, unresolved nonterminal row, circular census, or final negative pass that lacks source evidence or still finds candidates. Downgrade claims such as ready, works, accessible, robust, secure, deployable, beloved, viral, complete, or full Shipworthy unless directly supported.
...
- Read `references/coverage-manifest.md`, `references/evidence-state.md`, and `ship-product-workflows/references/path-discovery-and-coverage.md` before creating candidate inventories, coverage manifests, ledgers, the material-state control census, coverage matrices, path labels, evidence debt, or readiness claims.
...
For each finding include evidence, user consequence, likely cause, smallest useful fix, counterfactual/no-change baseline when relevant, downstream regression or simplification risk, and exact verification step. For full runs, include the source-backed coverage manifest for every candidate and expected intent, using the canonical frontier statuses and showing owner, variants, evidence, result, and terminal reason.

### Canonical artifact gate

Before the verifier approves or HTML is rendered: Re-open both final JSON files from disk, readiness-ledger.json and report-input.json. readiness-ledger.json contains the canonical top-level `path_frontier`; report-input.json is exactly the current `shipworthy/readiness-report-input` 1.1 wrapper, and its `source_ledger` must be structurally identical to readiness-ledger.json. Do not add a second top-level frontier or ad hoc checkpoint to the closed report wrapper. Every `Fix` finding needs non-empty `affected_semantic_keys`, a semantic `observed_effect_code`, and `evidence_refs`; evidence references are safe relative paths to existing non-empty files under the evidence output, optionally followed by a fragment. Every non-intent frontier row names its correct immediate parent and mechanically derives its semantic key from that parent. Execute both bundled schemas, compare source candidate inventories, frontier identities/counts/qualification/finality, and finding lineage, then render HTML from that exact report input. Missing or divergent canonical data fails completion.

Write the operational checkpoint separately as `orchestration-checkpoint.json` in the directory containing `report-input.json`; never inject it into the closed wrapper. Record the human target name, actual lane roster/status/evidence, execution mode, authorization, frontend path-walk, verifier, omissions, ledger/evidence locations, and exhaustion status. `scripts/render_report.py` is the final fail-closed gate: it checks frontier parents and finding lineage, evidence existence and available-artifact integrity, the computed frontier digest chain, and this checkpoint before writing HTML. Invalid canonical input must not be rendered and must remain unchanged.
