...
Before target analysis or lane dispatch, read the full SKILL.md bodies for ship-readiness-orchestrator, ship-deep-review, ship-product-workflows, and ship-workflow-clarity. Then run the Multi-Agent Authorization Gate: if the user has not already explicitly authorized parallel subagents, delegation, or multi-agent work, ask for that authorization before any subagent dispatch; if authorization is denied, unavailable, or not received, run the same roster sequentially and record "sequential fallback because multi-agent authorization was not granted." Then read the target request, repo/app instructions, source-of-truth docs, and runtime facts. Record the target fingerprint, safe-test boundary, and multi-agent authorization status before dispatching agents or tools. Initialize the canonical claim ledger, coverage matrix, evidence debt register, raw lane outputs, verifier outputs, and final drift check before lane dispatch. Write target fingerprint, safe boundary, multi-agent authorization status, lane roster, path universe, lane merges, verifier decisions, fix-cascade notes, and final dispositions into that state as the run proceeds.
