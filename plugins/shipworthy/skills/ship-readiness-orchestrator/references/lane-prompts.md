...
Start with expected-intent discovery, path discovery, path_frontier additions, and coverage mapping. Return reasonable user goals, missing expected paths, routes/screens/roles/states/actions/hidden paths/variants/integrations/data states/devices/mutation risks. Try or safely trace every safe discoverable path inside scope, including happy path, empty, loading, error, invalid input, back/forward, cancel, refresh, role mismatch, permission denial, responsive, and recovery states where in scope.
