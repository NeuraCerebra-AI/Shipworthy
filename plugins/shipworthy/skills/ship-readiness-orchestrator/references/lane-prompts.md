...
Derive frontier counts and closure from rows; do not trust caller claims. Shadow-read the raw interaction evidence and reject closure when a raw observation has no corresponding distinct input control, spawned surface, state-boundary transition, or finding lineage. Before approval, reconcile each observed input mechanism, each invalid/corrected/success boundary, and each reload/re-entry control and transition; require unavailable capabilities as feature rows, false affordances as surface rows, and one finding and semantic effect code per defect class. Re-open the final readiness-ledger.json and report-input.json from disk and reject approval unless each has the identical top-level `path_frontier`; schema-valid legacy compatibility is not sufficient, and every `Fix` finding requires `affected_semantic_keys`, `observed_effect_code`, and `evidence_refs`. Compare runtime structural evidence, including event listeners and keyboard handlers, to frontier input mechanisms and independently repeat the bounded conventional shortcut probe when the safe runtime is available. Every conventional shortcut that produced behavior requires its own control and transition rows even when pointer activation reached the same surface. A safe control on a material supplied role/state/viewport variant cannot be `sampled_with_justification` when that variant establishes responsive, permission, state, or input behavior; require direct transition evidence. Reject closure whenever a raw role/state/viewport observation or screenshot lacks its matching surface, control, and observed transition rows. Preserve every boundary in an invalid-to-valid retry as separate transitions. Persistence proof requires a distinct reload or re-entry control and transition; the reload result must not be folded into the original Save transition. An in-scope destructive control is `avoided`, not `out_of_scope`, when it was intentionally not invoked for safety. Reject `closed_multi_source` for unresolved material rows, summary/row count drift, unreconciled feature or surface differences, fewer than two qualifying zero-yield passes from distinct canonical method families, a `single_source`, `static_only`, `blocked`, or `incomplete` frontier, or covered controls/transitions without attempt and evidence proof. Fail the full-run claim if no browser/computer-use/frontend path-walk occurred while the report claims full Shipworthy. A source/CLI/HTTP-only readiness audit is not a full Shipworthy run; require a downgrade label and evidence debt instead. State explicitly whether the orchestrator may write the wave intelligence summary.
```
