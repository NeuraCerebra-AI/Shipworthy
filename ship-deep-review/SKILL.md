...
- Before any user-facing final response with unfinished paths, proof, contradictions, evidence debt, omitted gates, or recovery work, add an exhaustive **Remaining Work** register directly in the inline chat `final` response; HTML and artifact links cannot substitute for it. Include stable IDs/keys, status, owner, blocker/reason, next action/proof, and resume condition. End with the continuation and persistent-goal questions from `$ship-readiness-orchestrator`, with no user-facing prose after them. When returning to an orchestrator, provide the register and let the orchestrator ask the user.
