...
When the audit is not truly finished, the inline chat `final` response must include an exhaustive **Remaining Work** handoff derived from the canonical artifacts. Include every material `sampled_with_justification`, `blocked`, `avoided`, `evidence_debt`, `unattempted`, `unknown`, or `maybe` row; every inventory difference and debt item; every omitted gate; and every remaining safe action/resume condition. Show stable ID/key, status, owner, reason/blocker, next action/proof needed, and resume condition. If too large inline, write and link a complete `remaining-work.json`, but still include exact grouped counts, all unresolved categories, the next frontier batch, and both questions inline; never hide rows behind “and more.” HTML, ledger, attachment, or artifact-link presence does not satisfy the inline-chat gate.
